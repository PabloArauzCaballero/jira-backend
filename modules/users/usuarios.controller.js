...
async function updateUser(req, res){
    ...
    try{
        ...
        if (!validationResult.success || !validationId.success) {
            logger.warn(
                {
                    event: "users_update_validation_failed",
                    ...getRequestMeta(req),
                    durationMs: Date.now() - startedAt,
                    validationErrors: validationResult.error.flatten(),
                },
                "Validación fallida en actualizar usuario"
            );
        ...
        return res.status(500).json({
            success: false,
            message: "Error interno de actualizar usuario.",
        });
    }
}
...
async function deleteUser(req, res){
    ...
    try{
        ...
        if (!validationId.success) {
            logger.warn(
                {
                    event: "users_delete_validation_failed",
                    ...getRequestMeta(req),
                    durationMs: Date.now() - startedAt,
                    validationErrors: validationId.error.flatten(),
                },
                "Validación fallida en eliminar usuario"
            );
